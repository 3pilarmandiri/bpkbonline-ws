<?php 
class buku_pajak extends  master_controller {
	...
	$data['title'] = "CETAK BUKU PEMBANTU PAJAK ";
	...
	$this->render_baru();
  	...
  	function cetak(){
		...
		$pdf->AddPage('L');
		 ...
		 $html = $this->load->view("pdf/buku_pajak_table_data",$data,true);
		 $pdf->writeHTML($html, true, false, true, false, '');
		...
		// ---------------------------------------------------------
		 ...
		 if($halaman <> $pdf->getPage() ) {
		 	...
		 	$pdf->AddPage('L');
		 	$html = $this->load->view("pdf/buku_pajak_table_header",$data,true);
		 	$pdf->writeHTML($html, true, false, true, false, '');
